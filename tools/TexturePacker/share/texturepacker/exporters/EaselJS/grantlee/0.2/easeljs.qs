...
var printAnimations = function() {
    ...
    {
        var nameAndFrame = spriteList[i].trimmedName.match(/(\D+)[-_.\//](\d+)$/);
        if (nameAndFrame)
        {
        ...
        {
            animations[spriteList[i].trimmedName] = [i];
        }
    }
    var str = '';
    for (var name in animations)
    {
        str += '"' + name + '": [' + (animations[name]).filter(function(n){ return n !== undefined }).join(', ')+ '],\n    ';
    }
    return '"animations": {\n    ' + str.trim().slice(0,-1) + '\n},';
}
